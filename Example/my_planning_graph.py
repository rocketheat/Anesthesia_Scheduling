...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        # create a list that will contain all possible actions
        a_nodes = []
        # Loop over all actions
        for action in self.all_actions:
            # Create a PgNode_a instance with the action. This will give access
            # to methods that can check all effect nodes
            a_node = PgNode_a(action)
            if a_node.prenodes.issubset(self.s_levels[level]):
                a_nodes.append(a_node)
                # Check all literals at the level
                for s_node in self.s_levels[level]:
                    # Connecting the S node to the appropriate children/parents
                    s_node.children.add(a_node)
                    a_node.parents.add(s_node)
        self.a_levels.append(a_nodes)
        # The only output required is an action to append all
        return None
    ...
    def add_literal_level(self, level):
        ...
        # Per the AIMA 3rd edition page 388. Every Si level contains all the literals that could
        # result from any possible choice of actions in Ai-1
        # 1. Step to calculating all the literals:
        # 2. I need to start from Ai-1 as opposed to current level, this can be achieved with Action level-1
        # 3. from there I need to access all possible actions using the effnodes methods
        # 4. the book also mention that we don't require to choose actions in this step but rather we are just recording so we don't need to use an 'if statements' here

        s_nodes = set() # to store the nodes

        previous_level = level-1 # accessing the previous level
        for a_node in self.a_levels[previous_level]: # loop over all nodes in the Ai-1
            for s_node in a_node.effnodes: # for each a_node we get the possible child S nodes and loop over them
                s_nodes.add(s_node) # adding to the s_node
                s_node.parents.add(a_node) # making the child-parent connection similar to above
                a_node.children.add(s_node)

        # Adding all the nodes to the state
        self.s_levels.append(s_nodes)
        return None # no output for this function
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a): # bool:
        ...
        # Per AIMA 3rd edition page 388 and description above. Inconsistent effects: One action negates of another
        # if the effect are not in both paths (as in the example provided where the actions disagreed on the effect Have(cake))

        mutex = False

        # We start with False and if either node_a1 or node_a2 have actions that negate the other then it is True
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a): # -> bool:
        ...
        # assessing if:
        # 1. effects are not negated between node_a1 + and node_a2 -
        # 2. effects are not negated between node_a2 + and node_a1 -
        # 3. effects are not negated between node_a1 - and node_a2 +
        # 4. effects are not negated between node_a2 - and node_a1 +
        for action in node_a1.action.effect_add:
    ...
    def h_levelsum(self): # -> int:
        ...
        # Obtaining the levelsum heuristic

        # iterate over all problem goals
        for goal in self.problem.goal:
            # start with a False
            reached = False
            # We can then iterate over all self.s_levels which represents a list of sets where each set is a level.
            for depth, level in enumerate(self.s_levels):
                # Assess each state in each level
                for state in self.s_levels[depth]:
                    # if state.is_pos == True then add positive if false add negation
                    if state.is_pos:
                        literal = expr(state.symbol)
                    elif not state.is_pos:
                        literal = expr('~{}'.format(state.symbol))
                    if goal == literal:
                        reached = True
                        # if goal reached then convert reached to True and add depth to the sum of the level costs.
                        level_sum += depth
                        break
                if reached:
                    break
